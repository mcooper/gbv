...
data <- read.csv('G://My Drive/GBV/GBV_all.csv') %>%
  group_by(latitude, longitude) %>%
  summarize(perc12=mean(perc12, na.rm=T)) %>%
...
ggplot() + 
  ...
  coord_sf(xlim = c(max(data$longitude), min(data$longitude)), ylim = c(min(data$latitude), max(data$latitude))) + 
  theme_void()

ggsave('C://Users/matt/ipv-rep-tex/img/Map.pdf', width=10, height=3)
